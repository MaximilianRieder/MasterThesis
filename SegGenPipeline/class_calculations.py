...
def get_class_weights(train_loader, num_classes, device, five_crop):
    ...
    for i, data_package in enumerate(tqdm(train_loader), 0):
        #for data in data_package.values():
            
        mask = data_package["data_z"][1]
...
def get_unavailable_class_indices(train_loader, val_loader, num_classes, device, five_crop):
    ...
    ### For validation data.
    for i, data_package in enumerate(tqdm(val_loader), 0):
        #for data in data_package.values():
        mask = mask = data_package["data_z"][1]
