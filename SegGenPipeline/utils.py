...
import config_seg_gan as config
from PIL import Image

from torchvision import transforms

def save_image(image, idx, frame_path):
    mean = config.mean_smokeset_hs_z
    std = config.std_smokeset_hs_z
    img = image[idx]
    img = img.cpu().detach()
    img = np.array(img)
...
#######################
def scale_max_pixel(img, max_px):
    img_scaled = (img + 1.0) / 2.0
    return img_scaled * max_px

def normalize_tensor(img, mean, std, max_px):
    #img_norm = (img - mean) / std
    data_norm = torch.stack([
        (img[:, 0] - mean[0]) / (std[0] * max_px),
        (img[:, 1] - mean[1]) / (std[1] * max_px),
        (img[:, 2] - mean[2]) / (std[2] * max_px)
    ], dim=1)
    return data_norm
